...
<script type="text/javascript">
	...
	function form_edit(form) {
		var form_url = siteUtil.basePath+"/sysmgr/deptAction/dept_form.do" ;
		if("E" == form) {
			var node = $tg.treegrid('getSelected');
			if (node) {
				form_url = siteUtil.basePath+"/sysmgr/deptAction/dept_form.do?id="+node.id ;
			} else {
	...
	function del() {
		...
		if(node){
			$.messager.confirm("您确定要进行该操作？<br/>该删除操作会将子菜单一并删除。", function (c) { 
				if(c) {
					$.post(siteUtil.basePath+"/sysmgr/deptAction/delete.do", {id:node.id}, function(result) {
						if (result.status) {
							$tg.treegrid('reload') ;
