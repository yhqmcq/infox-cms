...
(function ($) {
    ...
    window.mainpage.bindToolbarButtonEvent = function () {
        ...
        $(btnLook).click(function () {
        	loginOut($(this).attr("name"));
        });
        
        $(btnExit).click(function (b) {
        	loginOut($(this).attr("name"));
        });
    };
    ...
    window.mainpage.setTheme = function (theme, setCookie) {
        ...
        $.easyui.theme(true, theme, function (item) {
            if (setCookie) {
                ...
                var msg = $.string.format("您设置了新的系统主题皮肤为：<br/><br/>{0}，{1}。", item.name, item.path);
    ...
    window.mainpage.online = function() {
    	var o = $(online).datagrid({
			...
			pagination: true,
			columns: [[
			    { field: 'account', title: '账号', width: 70, formatter:function(value,row){
			    	if(undefined != row.type && "" != row.type && row.type == 1) {
			    		return $.string.format("<a href='javascript:;'>{0}</a>", "<font color='green'>"+value+"</font>");
			    	} else {
			    		return value;
			    	}
			    }},
			    { field: 'truename', title: '姓名', width: 80 },
			    { field: 'ip', title: 'IP(ip)', width: 100 }
			]],
			enableHeaderClickMenu: false,        //此属性开启表头列名称右侧那个箭头形状的鼠标左键点击菜单
	        enableHeaderContextMenu: false,      //此属性开启表头列名称右键点击菜单
	        selectOnRowContextMenu: false      //此属性开启当右键点击行时自动选择该行的功能
	    });
    	o.datagrid('getPager').pagination({
    		layout:['first','prev','next','last','refresh'],
    		pageSize : 10,
			pageList : [ 10, 20, 30 ],
			displayMsg: "在线[{total}]人",
			onRefresh: function() {
			}
    	});
    	//展开east面板是重新加载在线用户
    	var eastLayout = $(mainLayout).layout("panel",'east'), opts = eastLayout.panel("options");
    	eastLayout.panel({
    		title: opts.title,
    		onExpand: function(){
				o.datagrid('reload');
